...
public class ListbyCategoryModel : PageModel
{
    ...
    public async Task OnGetAsync(int id = 0, int pagenumber = 1, string sortOrder = "name")
    {
        ...
        if (id > 0)
        {
            ...
            var currentFp = dbContext.PartCategory.FirstOrDefault(p => p.PCpkey == CurrentId);
            CurrentCategory = currentFp!.PCName;
            ViewData["Title"] = "Parts by Category " + CurrentCategory;

            // populate dropdown
            var fpList = dbContext.PartCategory.OrderBy(p => p.PCName).ToList();

            foreach (var record in fpList)
            {
                Options.Add(new SelectListItem { Text = record.PCName, Value = record.PCpkey.ToString(), Selected = currentFp.PCpkey == record.PCpkey });
            }

            Data = await PartService.GetPartsAsync(CurrentSort, skip, pageSize, dbContext, 0, 0, 0, CurrentId);

            var totalRecords = dbContext.Parts.Count(p => p.PartCategoryID == CurrentId);
            ...
            Pagination = Helpers.GetPagination(TotalPages, CurrentPage, "parts/listbycategory", "sortorder=" + CurrentSort + "&id=" + CurrentId);
        }
